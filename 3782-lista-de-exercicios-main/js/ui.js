...
const ui = {
  ...
  adicionarFilmeNaLista(filme) {
    ...
    botaoFavorito.onclick = async () => {
      try {
        await api.atualizarFavorito(filme.id, !filme.favorito); //Tem que negar porque negando, será colocado o oposto do que já está lá
        ui.renderizarFilmes();
      } catch (error) {
        console.error("Erro ao tentar atualizar favorito", error);
      }
